...
can only start after the first one is complete (Buy->sell->Buy->sell). 
...
The idea is to store the maximum possible profit of every subarray and solve the 
problem in the following two phases.
...
maximum profit such that profit[i] contains maximum achievable profit from two transactions 
in subarray price[0..i].
...
There is one more approach for calculating this problem using the Valley-Peak approach 
i.e. take a variable profit and initialize it with zero and then traverse through the array 
of price[] from (i+1)th position whenever the initial position value is greater than the previous 
value add it to variable profit.

But this approach does not work in this problem, that you can only buy and sell a stock two times. 
For example, if price [] = {2, 4, 2, 4, 2, 4} then this particular approach will give result 6 while 
in this given problem you can only do two transactions so answer should be 4. 
...
This is to ensure to get profit from transactions. Iterate through the array and return the second 
sell as it will store maximum profit.
