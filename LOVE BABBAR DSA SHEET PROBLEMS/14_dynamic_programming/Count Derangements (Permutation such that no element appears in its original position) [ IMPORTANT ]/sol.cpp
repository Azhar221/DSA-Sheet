...
appears in its original position) [ IMPORTANT ]
...
Let countDer(n) be count of derangements for n elements. 
Below is the recursive relation to it.  

countDer(n) = (n - 1) * [countDer(n - 1) + countDer(n - 2)]
How does above recursive relation work? 

There are n – 1 way for element 0 (this explains multiplication with n – 1). 
Let 0 be placed at index i. There are now two possibilities, 
depending on whether or not element i is placed at 0 in return. 
...
for n-1 elements as now there are n-1 elements, 
...
Time Complexity: O(2^n) since T(n) = T(n-1) + T(n-2) which 
is exponential.

Auxiliary Space: O(h) where h= log n is the maximum height 
of the tree.

We can observe that this implementation does repetitive work. 
For example, see recursion tree for countDer(5), countDer(3) is 
...
An Efficient Solution is to use Dynamic Programming to store 
results of subproblems in an array and build the array 
in bottom-up manner. 
...
instead of Storing the values in an array two variables 
can be used to just store the required previous only.
*/
